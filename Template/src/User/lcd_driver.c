...
void LCD_ShowInitUI()
{       
   LCD.Debug.SetCursor(40,0);
   LCD.Write.XLine(9,"Welcome.");
...
void LCD_Init()
{
    ...
    GPIO_PinInit(LCD_SDA_GPIO_PIN,GPO,0);      
    GPIO_PinInit(LCD_SCL_GPIO_PIN,GPO,0); 
    GPIO_PinInit(LCD_CS_GPIO_PIN,GPO,0);
...
#if SUPPORT_UART
                       //unsigned char chi = ch;
                       
                       for(int i = 0;i<2;i++)
                       {
                          if(uart[i].Is_Run &&uart[i].Handle.IsSendingData)
                          {
                              UARTx_SendStr(&uart[i],(unsigned char*)&ch);
                          }       
                       }
#endif                        
...
{	
	...
	LCD.Cursor.y=row*(LCD.Font.Hight);
...
/*						LCD Basic Info												*/
		...
		.Width = 128,																				//Width
		.Type = 1,																					//display_Type
		.Cursor = {0,0},																			//Cursor(x,y)
		.Backgroundcolor = WHITE,																			//background color
		.XLine = {ORANGE,1,8+4+2},														//XLine(color,Width,interval)
		.Font = {1,8,6,BLACK,WHITE,usingWriteXLine},							//Font(Type,Hight,Width)
/*						Function												*/
#if USING_FUNCTION_ENCAPSULATION
/*						下面这个结构体仅为上述函数的整理，用法在LCD_Test函数中，不用可注释掉，同时将所有static void 改为 void	*/
		.Init = LCD_Init,
                .DeInit = LCD_DeInit,
		.Show = {LCD_ShowChar,LCD_ShowNum,LCD_ShowChinese,LCD_ShowStr},
		.Draw = {LCD_DrawPoint,LCD_DrawHLine,LCD_DrawVLine,LCD_DrawLine,LCD_DrawSqr,LCD_DrawCircle,LCD_DrawPic},
		.Write = {LCD_WriteHalfLine,LCD_WriteLine,LCD_WriteXLine},
		.Printf = {printf},
#if USING_HARDWARE_CONTROLER
		.Debug = {LCD_HardWareWriteByte,LCD_HardWareWriteWord,LCD_HardWareWriteCmd,\
...
#else 
		.Debug = {LCD_SoftWareWriteByte,LCD_SoftWareWriteWord,LCD_SoftWareWriteCmd,\
		 LCD_Test,LCD_AddrReset,LCD_SetPos,LCD_Clear,LCD_Fill,LCD_SetCursor,LCD_SetBackGroundColor,\
		 LCD_SetFontSize,LCD_SetFontColor,LCD_SetFontBackColor,LCD_SetXLineColor,LCD_SetXLineWidth,LCD_SetXLineInterval,LCD_SetXLineVal},	
#endif
#endif
