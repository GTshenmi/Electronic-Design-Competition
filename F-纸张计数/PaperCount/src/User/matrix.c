...
_fuzzy_ctrl_type_t_ fuzzy_ctrl_type;

void Replace(float value1,float value2,_fuzzy_ctrl_type_t_ type)
{
  
      switch(type)
      {
          case NL_T:
            Fuzzy_Value.NL_LastValue = value2;
          case NM_T:
            Fuzzy_Value.NM_LastValue = value2;
          case NS_T:
            Fuzzy_Value.NS_LastValue = value2;
          case ZE_T:
            Fuzzy_Value.ZE_LastValue = value2;
          case PS_T:
            Fuzzy_Value.PS_LastValue = value2;
          case PM_T:
            Fuzzy_Value.PM_LastValue = value2;
          case PL_T:
            Fuzzy_Value.PL_LastValue = value2;
      }
      for(int i=0; i<7;i++ )
      {
          for(int j=0;j<7;j++)
          {
             if(fuzzyRuleKp[i][j] == value1)
              fuzzyRuleKp[i][j] = value2;
             if(fuzzyRuleKi[i][j] == value1)
               fuzzyRuleKi[i][j] = value2;
             if(fuzzyRuleKd[i][j] == value1)
               fuzzyRuleKd[i][j] = value2;
          }
      }
}
